 ...
 *  picked by Asaf. Source of truth is lib/voices.ts, a plain module the
 *  server TTS route can import too. The characters speak in these voices
 *  via /api/tutor's `speak` action (lib/speech/useSpeech.ts), falling
 *  back to the browser's Hebrew voice when Cartesia is unavailable. */
export const CHARACTERS: Record<
  CharacterId,
  { id: CharacterId; label: string; accent: string; cartesiaVoiceId: string }
> = {
  boy: { id: "boy", label: "החבר", accent: "var(--color-pink)", cartesiaVoiceId: CARTESIA_VOICES.boy },
  girl: { id: "girl", label: "החברה", accent: "var(--color-teal)", cartesiaVoiceId: CARTESIA_VOICES.girl },
};
